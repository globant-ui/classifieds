...
export class AuthenticationWindowService {
  ...
  public createWindow( url, name = 'Window', width = 430, height = 600, fullScreen: boolean ) {
    if ( fullScreen ) {
      let options = 'scrollbars=no,width=' + screen.width +
      ', height=' + screen.height + ', top=1, left=1';
      ...
      let options = 'scrollbars=no,width=' + width + ', height=' + height
      + ', top=' + t + ', left=' + l;
      let wind = window.open( url, name, options );
      wind.moveTo( l, t );
      return wind;
