...
export class MapData {
  ...
  public mapCardData(form, publishStatus = false) {
      let today = new Date();
      let locale = 'en-us';
      let cardCreatedDate = today.getDate() + '-' + today.toLocaleString(locale, { month: 'short' })
      + '-' + today.getFullYear();
      let address = (form.value['area'] && form.value['city'] && form.value['state']
      && form.value['country']) ? form.value['area'] + '-' + form.value['city']
      + '-' + form.value['state'] + '-' + form.value['country'] : '';
      let data = {
        ...
        YearOfPurchase: (form.value['YearOfPurchase'])
        ? parseInt(form.value['YearOfPurchase'], 10) : 0,
        Status: '',
        ...
        KmDriven: (form.value['KmDriven']) ? parseInt(form.value['KmDriven'], 10) : 0,
        DimensionWidth: (form.value['DimensionWidth'])
        ? parseInt(form.value['DimensionWidth'], 10) : 0,
        DimensionLength: (form.value['DimensionLength'])
        ? parseInt(form.value['DimensionLength'], 10) : 0,
        DimensionHeight: (form.value['DimensionHeight'])
        ? parseInt(form.value['DimensionHeight'], 10) : 0,
        TypeofUse: '',
        Type: (form.value['Type']) ? form.value['Type'] : '',
        Negotiable: (form.value['negotiable'] && form.value['negotiable'] !== '' )
        ? form.value['negotiable'] : false,
